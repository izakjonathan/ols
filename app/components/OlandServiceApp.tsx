...
const adminViewTitles: Record<AdminView, { eyebrow: string; title: string }> = {
  newOrders: { eyebrow: "", title: "Nye forespørgsler" },
  calendar: { eyebrow: "", title: "Kalender" },
  invoices: { eyebrow: "", title: "Faktura" },
  completed: { eyebrow: "", title: "Færdige ordrer" },
  services: { eyebrow: "", title: "Ydelser" },
  company: { eyebrow: "", title: "Firma" },
  employees: { eyebrow: "", title: "Timer" },
  settings: { eyebrow: "", title: "Backup" }
};
...
export default function OlandServiceApp({ mode = "frontend", employeeToken = "" }: { mode?: "frontend" | "backend" | "employee"; employeeToken?: string }) {
  ...
  const activeAdminTitle = adminViewTitles[adminView];
    ...
    <main className="relative min-h-screen overflow-hidden bg-black text-stone-50">
        ...
        <div className="mx-auto grid h-full max-w-3xl content-center gap-3">
          ...
          <div className="grid gap-3">
            <a href="tel:+4526848789" className="panel cta-card block p-4 sm:p-5">
              <div><h3 className="tight-card-title">Ring til Øland Service</h3></div>
            </a>
      ...
      {isBackend && <section id="admin" className="backend-safe-bottom backend-screen px-5 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl">
        <div className="admin-module-header">
          <div>
            {activeAdminTitle.eyebrow && <p className="eyebrow">{activeAdminTitle.eyebrow}</p>}
            <h2 className="admin-screen-title">{activeAdminTitle.title}</h2>
          </div>
        </div>
        {adminView === "newOrders" && <div className="admin-stat-grid">{[{ label: "Nye", value: orders.filter((order) => order.status === "Ny").length }, { label: "Bekræftet", value: orders.filter((order) => ["Bekræftet", "Planlagt", "I gang"].includes(order.status)).length }, { label: "Udført", value: orders.filter((order) => order.status === "Udført").length }, { label: "Faktura sendt", value: orders.filter((order) => order.status === "Faktura sendt").length }, { label: "Betalt", value: orders.filter((order) => order.status === "Betaling modtaget").length }, { label: "Værdi", value: kr(kpis.value) }, { label: "Ikke betalt", value: kr(kpis.unpaid) }].map((item) => <div key={item.label} className="admin-stat-card"><p>{item.label}</p><strong>{item.value}</strong></div>)}</div>}
  ...
  return <main className="min-h-screen bg-black px-4 pb-10 pt-5 text-stone-50">
    <section className="mx-auto grid max-w-3xl gap-4">
      ...
      <section className="panel p-4">
        ...
        <p className="mt-2 text-2xl font-semibold text-white">{formatHours(totalHours)}</p>
      </section>
  ...
  return <div className="mt-4 grid gap-4">
    <section className="grid gap-2">
      <button type="button" onClick={openAllTasks} className={`w-full rounded-2xl border px-4 py-3 text-left transition ${allTaskListOpen ? "border-white bg-white/10" : "border-white/10 bg-white/[0.04] hover:border-white/35"}`}>
        <p className="text-xs uppercase tracking-[0.18em] text-stone-400">Total timer</p>
        <p className="mt-1 text-2xl font-semibold text-white">{formatHours(totalHours)}</p>
      </button>
...
function NewOrdersModule({ orders, onAccept, ...handlers }: { orders: Order[]; onAccept: (id: string) => void } & OrderEditHandlers) {
  return <section className="backend-module grid gap-4">
    <div className="flex justify-end"><span className="rounded-full border border-white/24 px-3 py-1.5 text-xs font-black uppercase tracking-[0.10em] text-white">{orders.length} nye</span></div>
...
function CompletedOrdersModule({ orders, ...handlers }: { orders: Order[] } & OrderEditHandlers) {
  ...
  const renderGroup = (title: string, list: Order[]) => <section className="grid gap-3"><h4 className="panel-title">{title}</h4>{list.length === 0 && <div className="panel p-5 text-stone-300/70">Ingen ordrer i denne gruppe.</div>}{list.map((order) => <details key={order.id} className="submitted-order-card panel p-0"><summary className="cursor-pointer list-none p-5"><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center"><div><p className="eyebrow">{order.id}</p><h5 className="text-xl uppercase tracking-[0.12em] text-white">{order.customer.name || "Kunde"}</h5><p className="text-stone-300/70">{order.status} · {order.paymentStatus}</p></div><b className="text-2xl text-white">{kr(orderTotal(order))}</b></div></summary><div className="submitted-order-detail border-t border-white/10 p-4"><div className="new-order-detail"><OrderDetail selectedOrder={order} {...handlers} /></div></div></details>)}</section>;
  return <section className="backend-module grid gap-6">{renderGroup("Faktura sendt / afventer betaling", invoiceSent)}{renderGroup("Betaling modtaget", paid)}</section>;
}
...
function ServiceGroup({ title, kind, items, onUpdate, onAdd }: { title: string; kind: "services" | "extras" | "packages"; items: Array<Service | Extra | CarePackage>; onUpdate: (kind: "services" | "extras" | "packages", id: string, patch: Partial<Service & Extra & CarePackage>) => void; onAdd: (kind: "services" | "extras" | "packages") => void }) {
  return <section className="panel p-5"><div className="mb-4 flex items-center justify-between gap-3"><h4 className="panel-title mb-0">{title}</h4><button type="button" className="outline-button" onClick={() => onAdd(kind)}>+ Tilføj</button></div><div className="grid gap-3">{items.map((item) => {
    ...
    return <details key={item.id} className="backend-order-card"><summary className="cursor-pointer list-none p-4"><div className="flex items-center justify-between gap-3"><strong className="text-white">{name}</strong><span className="text-stone-200">{kr(item.price)}</span>{item.draft && <em className="rounded-full bg-white px-2 py-1 text-[0.62rem] font-black uppercase tracking-[0.12em] text-black">kladde</em>}</div></summary><div className="grid gap-3 border-t border-white/10 p-4 sm:grid-cols-2"><Field label="Navn"><TextInput value={name} onChange={(e) => onUpdate(kind, item.id, isPackage ? { title: e.target.value } : { name: e.target.value })} /></Field><Field label="Pris"><TextInput type="number" value={item.price} onChange={(e) => onUpdate(kind, item.id, { price: Number(e.target.value) || 0 })} /></Field>{isPackage && <Field label="Ikon"><Select value={(item as CarePackage).icon} onChange={(e) => onUpdate(kind, item.id, { icon: e.target.value as CarePackage["icon"] })}><option value="shield">Shield</option><option value="diamond">Diamond</option><option value="crown">Crown</option><option value="laurel">Laurel</option></Select></Field>}<div className="sm:col-span-2"><Field label={isPackage ? "Indhold - én linje per punkt" : "Beskrivelse"}><TextArea value={desc} onChange={(e) => onUpdate(kind, item.id, isPackage ? { items: e.target.value.split("\n").filter(Boolean), description: e.target.value } : { description: e.target.value })} /></Field></div>{kind === "extras" && <Field label="Prisnote"><TextInput value={(item as Extra).note ?? ""} onChange={(e) => onUpdate(kind, item.id, { note: e.target.value })} placeholder="fx fra 200 kr." /></Field>}</div></details>;
  })}</div></section>;
}
