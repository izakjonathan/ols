...
const adminViewTitles: Record<AdminView, { eyebrow: string; title: string }> = {
  newOrders: { eyebrow: "Backend", title: "Nye forespørgsler" },
  calendar: { eyebrow: "Backend", title: "Kalender" },
  invoices: { eyebrow: "Backend", title: "Faktura" },
  completed: { eyebrow: "Backend", title: "Færdige ordrer" },
  services: { eyebrow: "Backend", title: "Ydelser" },
  company: { eyebrow: "Backend", title: "Firma" },
  employees: { eyebrow: "Backend", title: "Timer" },
  settings: { eyebrow: "Backend", title: "Backup" }
};
    ...
    <main className="relative min-h-screen overflow-hidden bg-black text-stone-50">
      ...
      {isBackend && <section id="admin" className="backend-safe-bottom px-5 pt-6 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl">
        <div className="admin-module-header">
          <div>
            <p className="eyebrow">{activeAdminTitle.eyebrow}</p>
            <h2 className="admin-screen-title">{activeAdminTitle.title}</h2>
          </div>
          <a href="/" className="outline-button admin-header-link">Kundeside</a>
        </div>
        ...
        <div className="admin-shell">
          ...
          {adminView === "settings" && <div className="backend-module grid gap-4 lg:grid-cols-2"><section className="panel p-6 lg:col-span-2"><h3 className="panel-title">Supabase</h3><p className="text-stone-300/75">Status: {syncStatus}</p><p className="mt-3 text-sm text-stone-400/80">Medarbejdere, timer, ordrer, fakturaer, ydelser og firmaoplysninger gemmes i Supabase, når miljøvariablerne er sat i Vercel. Ellers bruges lokal browserbackup.</p></section><section className="panel p-6"><h3 className="panel-title">Backup</h3><p className="text-stone-300/75">Eksporter alle ordrer til JSON, så data kan gemmes eller flyttes til en anden browser.</p><button className="gold-button mt-5 w-full" onClick={exportOrders}>Eksporter ordrer</button></section><section className="panel p-6"><h3 className="panel-title">Importer</h3><p className="text-stone-300/75">Importer en tidligere JSON backup. Dette erstatter de nuværende lokale ordrer.</p><input ref={importRef} type="file" accept="application/json" className="hidden" onChange={(e) => importOrders(e.target.files?.[0])} /><button className="outline-button mt-5 w-full" onClick={() => importRef.current?.click()}>Importer backup</button></section></div>}
